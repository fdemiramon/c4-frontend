...
export function Board({ boardIndex }: BoardProps) {
  const { gameEvent } = useGameEvents(boardIndex);
  ...
  if (!gameState && !loading) {
    return (
      <div className="bg-white rounded-lg p-4 border-2 border-yellow-500">
        <p className="text-yellow-500">No game data available for Board #{boardIndex}</p>
      </div>
    );
  }
    ...
    >
      <div className={`bg-indigo-600 p-4 rounded-t-lg text-white`}>
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="font-semibold">
            Game #{loading ? "..." : gameState?.gameIndex ?? "N/A"}
          </span>
        </div>
        <div className="flex items-center gap-2 bg-indigo-500/30 p-2">
          ...
          <span className="text-sm font-medium">
            {loading ? "Loading..." : `LATEST EVENT: ${gameEvent?.name ?? "None"}`}
          </span>
        </div>
      </div>
      <div className="space-y-4">
        <div className="bg-white rounded-lg shadow p-4">
          <BoardGrid key={gameState?.numberOfPlays} gameState={gameState} />
        </div>
      </div>
    </div>
  );
